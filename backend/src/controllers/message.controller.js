...
export const setupSocketEvents = () => {
  io.on("connection", (socket) => {
    ...
    // Kullanıcı mesajları okuduysa bu event tetiklenir
    socket.on("markMessagesAsRead", async ({ senderId, receiverId }) => {
      try {
        ...
        if (senderSocketId) {
          io.to(senderSocketId).emit("messagesMarkedAsRead", { by: receiverId });
        }
      } catch (err) {
        console.error("markMessagesAsRead hatası:", err);
      }
    });
  });
};
